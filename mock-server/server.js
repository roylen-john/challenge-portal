...
function isAuthenticated({ emp_id, password }) {
  ...
  return userdb.users.findIndex(user => user.emp_id === emp_id && user.password === password) !== -1
}
...
server.post('/auth/register', (req, res) => {
  ...
  fs.readFile("./users.json", (err, data) => {
    ...
    //Add new user
    const newUser = { id: last_item_id + 1, emp_id: emp_id, password: password, name: name }
...
server.post('/auth/login', (req, res) => {
  ...
  const user = userdb.users.find(user => user.emp_id === emp_id && user.password === password)
